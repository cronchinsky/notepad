...
if ($confirm && $qid) {
  $responses = $DB->get_records('notepad_question_responses',array('qid' => $qid));
  if ($responses) {
    $DB->delete_records_list('notepad_question_responses', 'id', array_keys($responses));
  }
  $DB->delete_records('notepad_questions', array('id' => $qid));
  
  redirect("editquestions.php?sid=$sid");
}

// Set the page header.
$PAGE->set_url('/mod/notepad/deletequestion.php', array('qid' => $qid, 'sid' => $sid));
$PAGE->set_title(format_string("Delete Question."));
...
echo $OUTPUT->confirm("Are you sure you want to delete this question?  Any responses will be lost.","deletequestion.php?qid=$qid&sid=$sid&confirm=1","view.php?n=$notepad->id");
